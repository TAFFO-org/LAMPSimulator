...
run_one()
{
  ...
  $TASKSET $flt_out &> $datadir/$benchname.float.csv > $datadir/$benchname.float.time.txt || return $?
  for ((i=1; i<$times; i++)); do
    $TASKSET $flt_out 2> /dev/null >> $datadir/$benchname.float.time.txt || return $?
  done
  $TASKSET $fix_out &> $datadir/$benchname.csv > $datadir/$benchname.time.txt || return $?
  for ((i=1; i<$times; i++)); do
